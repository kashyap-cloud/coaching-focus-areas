-- =================================================================
-- Coaching Focus Areas - Complete Database Schema
-- =================================================================

-- 1. USERS TABLE
-- Tracks unique users (session or device-based, no auth required)
CREATE TABLE IF NOT EXISTS users (
  id          TEXT PRIMARY KEY,              -- external user_id from handshake
  display_name TEXT,                         -- optional user-set name
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- 2. COACHING AREAS TABLE (reference / seed data)
CREATE TABLE IF NOT EXISTS coaching_areas (
  id          TEXT PRIMARY KEY,              -- e.g. 'career', 'executive'
  name        TEXT NOT NULL,
...
-- 3. EXERCISES TABLE (reference / seed data)
CREATE TABLE IF NOT EXISTS exercises (
  id              TEXT PRIMARY KEY,          -- template id, e.g. 'grow', 'swot-analysis'
  title           TEXT NOT NULL,
  description     TEXT,
  exercise_type   TEXT NOT NULL DEFAULT 'template',  -- 'template' | 'special'
  special_type    TEXT                       -- quiz | checkin | planner | etc. (only for special)
);

-- 4. COACHING_AREA_EXERCISES (join: which exercises belong to which area)
...
-- 5. EXERCISE_SESSIONS TABLE
-- One row per user attempt at an exercise (supports multiple attempts)
...
CREATE INDEX IF NOT EXISTS idx_exercise_sessions_area ON exercise_sessions(coaching_area_id);
CREATE INDEX IF NOT EXISTS idx_exercise_sessions_exercise ON exercise_sessions(exercise_id);

-- 6. EXERCISE_RESPONSES TABLE
-- Stores individual field answers for template-based exercises
CREATE TABLE IF NOT EXISTS exercise_responses (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id      UUID NOT NULL REFERENCES exercise_sessions(id) ON DELETE CASCADE,
  field_id        TEXT NOT NULL,             -- e.g. 'vision', 'goal', 'strengths'
  field_label     TEXT,                      -- snapshot of the label at time of saving
  field_type      TEXT,                      -- 'text' | 'list' | 'table' | 'rated-list'
  response_text   TEXT,                      -- for text/list type answers (JSON array for list)
  response_json   JSONB,                     -- for table rows, rated-list, or complex data
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (session_id, field_id)              -- one answer per field per session
);

CREATE INDEX IF NOT EXISTS idx_exercise_responses_session ON exercise_responses(session_id);

-- 7. SPECIAL_EXERCISE_RESPONSES TABLE
-- Stores full answers for special exercises (quiz, checkin, planner, etc.)
-- Uses JSONB blob for flexibility since each special type has very different data
CREATE TABLE IF NOT EXISTS special_exercise_responses (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id      UUID NOT NULL REFERENCES exercise_sessions(id) ON DELETE CASCADE,
  special_type    TEXT NOT NULL,             -- 'quiz' | 'checkin' | 'planner' | etc.
  response_data   JSONB NOT NULL DEFAULT '{}', -- full exercise data as JSON blob
  score           INT,                       -- optional score (for quiz)
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (session_id)
);
...
-- =================================================================
-- SEED DATA: Exercises (template-based)
-- =================================================================
...
INSERT INTO exercises (id, title, exercise_type, special_type) VALUES
  ...
  ('smart-goals-exercise','SMART! Goals',           'special', 'smart-goals')
ON CONFLICT (id) DO NOTHING;

-- =================================================================
-- SEED DATA: Coaching Area <-> Exercise mappings
-- =================================================================
INSERT INTO coaching_area_exercises (coaching_area_id, exercise_id, display_order) VALUES
  ('career',        'improvement-canvas',     0),
  ('career',        'swot-analysis',          1),
  ('career',        'not-to-do-list',         2),
  ('career',        'grow',                   3),
  ('executive',     'grow',                   0),
  ('executive',     'improvement-canvas',     1),
  ('executive',     'swot-analysis',          2),
  ('executive',     'not-to-do-list',         3),
  ('wellness',      'self-care-quiz',         0),
  ('wellness',      'self-care-checkin',      1),
  ('wellness',      'make-heart-sing',        2),
  ('wellness',      'too-comfortable',        3),
  ('leadership',    'detox-relationship',     0),
  ('leadership',    'make-heart-sing',        1),
  ('leadership',    'energy-zappers',         2),
  ('leadership',    'putting-up-with',        3),
  ('finance',       'urgent-vs-important',    0),
  ('finance',       'plan-do-review',         1),
  ('finance',       'my-money-priorities',    2),
  ('finance',       'financial-goals-chart',  3),
  ('performance',   'success-planner',        0),
  ('performance',   'stop-procrastinating',   1),
  ('performance',   'unstick-yourself',       2),
  ('performance',   'love-your-weaknesses',   3),
  ('mindset',       'three-month-vision',     0),
  ('mindset',       'get-motivated',          1),
  ('mindset',       'build-a-bridge',         2),
  ('mindset',       'smart-goals',            3),
  ('spiritual',     'mentor-magic',           0),
  ('spiritual',     'intuition',              1),
  ('spiritual',     'love-loathe',            2),
  ('spiritual',     'powerful-questions',     3),
  ('mental-health', 'let-go',                 0),
  ('mental-health', 'too-comfortable',        1),
  ('mental-health', 'discover-yourself',      2),
  ('mental-health', 'gratitude-diary',        3),
  ('transform',     'wacky-wild-goal',        0),
  ('transform',     'big-rocks',              1),
  ('transform',     'rocking-chair',          2),
  ('transform',     'goals-motivator',        3),
  ('communicate',   'talk-to-yourself',       0),
  ('communicate',   '35-questions-to-ponder', 1),
  ('communicate',   'positive-visualisation', 2),
  ('communicate',   'softened-start-up',      3),
  ('organization',  'get-perspective',        0),
  ('organization',  'career-discovery',       1),
  ('organization',  'career-discovery',       2),
  ('organization',  'priority-matrix',        3),
  ('creativity',    'dictionary-story',       0),
  ('creativity',    'alternate-uses',         1),
  ('creativity',    'my-day',                 2),
  ('creativity',    'compound-collaborative', 3),
  ('employee',      'daily-success-habits',   0),
  ('employee',      'reminder-list',          1),
  ('employee',      'intake-questions',       2),
  ('employee',      'take-stock-take-action', 3),
  ('corporate',     'reclaim-your-power',     0),
  ('corporate',     'spheres-of-influence',   1),
  ('corporate',     'build-a-bridge',         2),
  ('corporate',     'three-month-vision',     3),
  ('confidence',    'daily-success-habits',   0),
  ('confidence',    'priority-matrix',        1),
  ('confidence',    'positive-visualisation', 2),
  ('confidence',    'career-discovery',       3)
